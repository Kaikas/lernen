...
# Step 1: Add an aggregate query for category counts.
# GROUP BY is an important SQL skill for summaries and simple reports.
def get_category_summary():
    ...
    summary = conn.execute(
        ...
        SELECT
            categories.id,
            categories.name,
            COUNT(syntax_notes.id) AS note_count
        FROM categories
        LEFT JOIN syntax_notes ON syntax_notes.category_id = categories.id
...
# Step 2: Add a route for the summary page.
# The application now has a small reporting view next to the main CRUD screens.
@app.get("/summary")
def summary():
    return render_template("summary.html", summary_rows=get_category_summary())
